...
| --- | --- |
| `DIRECTIVE_CHANGE` (user instruction adds a rule / redesigns an experiment / changes metric·baseline·scope) — not an artifact, so `scan-events` will not catch it; propagate by hand | the directive's typed home (a `data/rules.js` registry row via `/research-op insert --target rule`, or the owning plan/scope surface) + tracker Resume Block `lastAction`/`workflow-state` + registry `lastUpdated` |
