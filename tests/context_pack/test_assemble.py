...
def _inputs(**over):
    base = {
        "project_node": {
            ...
            "spec": {
                ...
                "contributions": [
                    ...
                    "Bind accepted Scope Experiments to package execution through provenance.",
                ],
                "out_of_scope": [
        ...
        "experiment_nodes": [
            {
                "id": "experiment/retrieval-v2/M0-baseline-validity",
                "spec": {
                    "purpose": "Reproduce the baseline before testing the retrieval variant.",
            ...
            "sourceExperiments": [
                {
                    "id": "experiment/retrieval-v2/M0-baseline-validity",
                    "version": 1,
                    "source": "triage:experiment-baseline",
                }
            ],
        },
        "pending_scope": [
...
def test_scope_boot_sections_carry_project_tasks_and_provenance():
    ...
    assert {"project", "direction", "experiments", "package_provenance", "pending_scope"} <= keys
    assert "Build an auditable retrieval workflow" in md
    assert "Do not claim production adoption" in md
    assert "experiment/retrieval-v2/M0-baseline-validity" in md
...
def test_stamp_records_authoritative_event_position():
    pack = context_pack.assemble(_inputs(source_seq=42, source_hash="abc"))
    assert pack.stamp["source_seq"] == 42
    assert pack.stamp["source_hash"] == "abc"
