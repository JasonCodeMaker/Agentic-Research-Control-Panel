...
SCOPE_SCHEMA = {
    "levels": {
        "project": {
            ...
            "primary": ["goal"],
            "fields": {
                "goal": {"kind": "text", "label": "Goal", "minWords": 3, "maxWords": 100},
        ...
        "experiment": {
            "order": ["purpose", "config_ref", "gate", "control_mode"],
            "primary": ["purpose", "control_mode"],
            "fields": {
                "purpose": {"kind": "text", "label": "Experiment", "minWords": 20, "maxWords": 100},
                "config_ref": {"kind": "ref", "label": "Config"},
...
def test_active_tree_built_from_parents_not_level_names():
    recs = [
        ...
        {"node_id": "exp/t1", "node": {"id": "exp/t1", "level": "experiment", "status": "ACTIVE", "parents": ["dir/a"]}},
        {"node_id": "exp/multi", "node": {"id": "exp/multi", "level": "experiment", "status": "ACTIVE", "parents": ["dir/a", "dir/b"]}},
    ]
    ...
    # a multi-homed experiment appears under each active parent
    assert "exp/multi" in tree["childrenOf"]["dir/a"]
    assert "exp/multi" in tree["childrenOf"]["dir/b"]
    assert tree["childrenOf"]["dir/a"] == ["exp/multi", "exp/t1"]  # sorted, stable
...
def test_triage_fold_separates_pending_from_disposed():
    recs = [
        ...
        {"id": "p2", "change": "add experiment", "status": "pending"},
...
def test_triage_fold_preserves_proposal_fields_after_disposition():
    recs = [
        {"id": "p2", "change": "add experiment", "status": "pending", "proposed_node": {"id": "exp/t1"}},
        {"id": "p2", "status": "accepted"},
    ]
    res = _run("return SI.foldTriage(%s);" % json.dumps(recs))
    assert res["accepted"][0]["change"] == "add experiment"
    assert res["accepted"][0]["proposed_node"]["id"] == "exp/t1"
...
def test_schema_health_reports_old_keys_missing_fields_and_word_counts():
    ...
    assert any("goal" in m and "3-100" in m for m in messages)
...
def test_current_understanding_counts_active_scope_and_package_links():
    projection = {
        ...
        "exp/t1": {"id": "exp/t1", "level": "experiment", "status": "ACTIVE",
                   "parents": ["dir/a"], "spec": {"purpose": "Run baseline reproduction."}},
        "exp/old": {"id": "exp/old", "level": "experiment", "status": "ARCHIVED",
                    "parents": ["dir/a"], "spec": {"purpose": "Old run."}},
    }
    triage = {"pending": [{"id": "p1"}], "accepted": [], "rejected": [], "disposed": []}
    packages = [
        {
            "id": "pkg-a",
            "sourceDirection": "dir/a",
            "sourceExperiments": [{"id": "exp/t1"}],
            "experiments": [{"id": "P0", "aliases": ["exp/t1"]}],
        },
        {
            "id": "pkg-b",
            "sourceExperiments": [{"id": "exp/t1"}],
            "experiments": [{"id": "P1", "aliases": ["exp/t1"]}],
        },
    ]
    res = _run(
...
def test_package_readiness_guides_scope_first_package_creation():
    projection = {
        ...
        "dir/c": {"id": "dir/c", "level": "direction", "status": "ACTIVE",
                  "parents": ["project/main"], "spec": {}},
        "exp/a/m0": {"id": "exp/a/m0", "level": "experiment", "status": "ACTIVE",
                     "parents": ["dir/a"], "spec": {}},
        "exp/c/m0": {"id": "exp/c/m0", "level": "experiment", "status": "ACTIVE",
                     "parents": ["dir/c"], "spec": {}},
    }
    triage = {"pending": [
        {"id": "exp-b", "proposed_node": {"id": "exp/b/m0", "parents": ["dir/b"]}},
    ]}
    ...
    assert by_id["dir/a"]["experimentCount"] == 1
    assert by_id["dir/b"]["state"] == "pending_experiments"
    assert by_id["dir/b"]["pendingExperimentCount"] == 1
...
def test_linked_packages_include_formal_source_experiment_alias():
    packages = [
        {
            "id": "pkg-a",
            "sourceDirection": "dir/a",
            "sourceExperiments": [{"id": "exp/t1"}],
            "experiments": [{"id": "exp/t1", "local_id": "P0"}],
        },
        {
            "id": "pkg-b",
            "sourceExperiments": [{"id": "exp/t1"}],
            # A legacy alias remains readable during migration.
            "experiments": [{"id": "P1", "aliases": ["exp/t1"]}],
        },
    ]
    res = _run("return SI.linkedPackages(%s, %s);" % (json.dumps("exp/t1"), json.dumps(packages)))
    assert [p["id"] for p in res] == ["pkg-a", "pkg-b"]
    assert res[0]["matchedExperiments"][0]["id"] == "exp/t1"
...
def test_package_provenance_health_reports_missing_scope_links():
    ...
    packages = [
        {
            ...
            "sourceExperiments": [{"id": "exp/missing"}],
            "experiments": [{"id": "P0", "aliases": []}],
        }
    ]
    ...
    assert any("sourceExperiments" in m and "exp/missing" in m for m in messages)
    assert any("canonical-id link" in m and "exp/missing" in m for m in messages)
