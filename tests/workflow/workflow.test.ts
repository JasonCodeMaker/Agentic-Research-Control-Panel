...
test("routes plan revision explicitly instead of terminating", () => {
  ...
  assert.equal(ticket.workflowState, "BLOCKED");
...
test("keeps dashboard schema constants in sync with workflow schema", () => {
  const dashboardSchema = readFileSync("skills/research-dashboard/assets/dashboard/data/schema.js", "utf-8");
  const routeMatch = dashboardSchema.match(/window\.NEXT_ROUTE\s*=\s*(\[[^\]]+\])/);
  assert.ok(routeMatch);
  assert.deepEqual(parseJsStringArray(routeMatch[1]), workflowSchema.nextRoute);

  const statusBlock = dashboardSchema.match(/"in-progress":\s*\{[\s\S]*?states:\s*(\[[\s\S]*?\])/);
  assert.ok(statusBlock);
  assert.deepEqual(parseJsStringArray(statusBlock[1]), workflowSchema.workflowStates);
});
