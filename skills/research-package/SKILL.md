...
| `experiments` | `--experiments <json>` at scaffold time, then `/research-op insert/update --target experiments-row` | Typed task spine array `[{id, label?, purpose, after, output, gate, status, measures, requiresCode, complex, resultSchemaRef?, runLink?, docsAnchor?}]` painted onto both `index.html#plan-status` (status chips by `renderPlanStatus()`) and `plan.html#experiments` (pipeline timeline by `renderPipelineTimeline()`). `measures` defaults true for new tasks; infra/setup tasks set `measures: false`. See [Pipeline timeline](#pipeline-timeline-binding) for caps. For each measurable task, the agent must design the result table from the task question: comparison axis, metric columns, gate selector, and provenance. Scaffold accepts inline `resultSchema` on an experiment row, stores the detailed schema under `<pkg>.facts.js resultSchemas`, and writes `resultSchemaRef` back to the inventory row. The scaffolders and research-op derive result slots, result-gate rows, change-card stubs, docs/pipeline blocks, and tracker to-dos from this spine. Update the matching entry's `status` whenever a phase opens/closes (same turn as the tracker row update). Allowed `status`: `pending`/`queued`/`running`/`completed`/`failed`/`skipped`/`blocked`. |
...
2. **Spine first** — author `experiments[]` before page content: `id`, action-verb `purpose` (&le;12 words), `after` DAG, one `output`, one `gate`, and `measures`/`requiresCode`/`complex`. For every `measures: true` row, also design a task-specific `resultSchema`: the decision question, row axis, metric columns, primary gate selector, and required provenance. Example: a multi-baseline Recall task uses baseline rows and Recall@K columns; sweeps use config rows and primary/budget columns.
...
- `learnings_lint.py alignment` enforces:
  ...
  - for `measures: true`, one result-gate row, one task-specific schema in `<pkg>.facts.js resultSchemas`, and one predefined fact-backed table `data-table="result_table_<id>"` with a `result-slot-<id>` anchor for compatibility;
...
**Top-of-doc index and timeline.** Every doc renders a top-of-page index from its own `h2`/`h3` headings, with anchor links that jump to each section. Both doc templates carry an empty `<nav class="docs-toc" data-docs-toc>` host that the bundled `assets/toc.js` fills client-side once a doc passes about four headings; shorter docs render nothing. Set `data-docs-toc-min="N"` on the host to change that threshold. Anchor ids are derived automatically and stay stable: an existing `id` on the wrapping `<section>`/`<article>`, or on the heading itself (as in `pipeline.html`'s `<h3 id="p1">`), is reused before a slug is generated, so hand-authored anchors keep working and no authoring step is needed. Mark dated log sections with `data-ts="YYYY-MM-DD"` (or `class="timeline-entry"`, or a dated span such as `<h2>Title <span class="muted">(2026-06-25)</span></h2>`, where the date is read from the text) and the index gains a chronological timeline beneath it; docs with no dated sections show only the index. The `.docs-toc` container lives in `assets/toc.css` and the timeline reuses the shared `.timeline` primitive, so it adds no new block classes. A doc template that replaces the bundled shell keeps the `[data-docs-toc]` host and the `assets/toc.css` / `assets/toc.js` includes so the index still renders.
