...
The **Agent Context** page (`<root>/context.html`, also bundled, rendered by `assets/research-context.js`) is a second derived view — over `data/context-core.js` (`window.RESEARCH_CONTEXT_CORE`) — that shows the compiled cross-package knowledge (learned rules, failed methods, adopted wins) the auto-research agent loads into its Context Pack at context-load. It is the model↔user context-alignment surface; `data/context-core.js` is regenerated by `lib/context_pack/build.py` and must not be hand-edited — edit the upstream stores instead.
